...
from validators.auth import RegisterForm,LoginForm
...
class Auth:
        ...
        if request.method == 'POST':
            if form.validate():
                ...
                if not user:
                    ...
                    flash('ایمیل یا نام کاربری تکراری است', 'warning')
    ...
    def Login(self):
        ...
        if request.method == 'POST':
            if form.validate():
                emailOrUsername = request.form.get('emailOrUsername')
                password = request.form['password']
                user = db.session.query(User.Users).filter(
                    or_(
                    User.Users.email == emailOrUsername,
                    User.Users.username==emailOrUsername)).first()
                if not user:
                    flash('اطلاعات اشتباه هستند', 'danger')
